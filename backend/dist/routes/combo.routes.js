...
const CMS_CONFIG_PATH = path_1.default.join(__dirname, '../../data/combos-cms-config.json');
...
function readCmsConfig() {
    try {
        ensureDataDir();
        if (fs_1.default.existsSync(CMS_CONFIG_PATH)) {
            const data = fs_1.default.readFileSync(CMS_CONFIG_PATH, 'utf-8');
...
function writeCmsConfig(config) {
    try {
        ensureDataDir();
        fs_1.default.writeFileSync(CMS_CONFIG_PATH, JSON.stringify(config, null, 2), 'utf-8');
        return true;
    }
    catch (error) {
...
// ============================================================
router.get('/cms/admin', auth_1.protect, (0, auth_1.authorize)('ADMIN'), async (_req, res, next) => {
    try {
        const config = readCmsConfig();
...
// ============================================================
router.put('/cms', auth_1.protect, (0, auth_1.authorize)('ADMIN'), async (req, res, next) => {
    try {
...
// ============================================================
router.put('/cms/:section', auth_1.protect, (0, auth_1.authorize)('ADMIN'), async (req, res, next) => {
    try {
        ...
        res.json({ success: true, message: `Section "${section}" updated`, data: config[section] });
    }
    catch (error) {
        next(error);
    }
...
router.get('/:slug', async (req, res, next) => {
    try {
        ...
        if (['cms', 'admin', 'stats'].includes(slug))
            return next();
        const combo = await database_1.prisma.comboProduct.findUnique({
            where: { slug },
            include: {
                primaryProduct: {
...
// ============================================================
router.get('/admin/all', auth_1.protect, (0, auth_1.authorize)('ADMIN', 'STAFF'), async (req, res, next) => {
    try {
        const combos = await database_1.prisma.comboProduct.findMany({
            include: {
                primaryProduct: {
...
// ADMIN: Create combo
// ============================================================
router.post('/admin', auth_1.protect, (0, auth_1.authorize)('ADMIN'), async (req, res, next) => {
    try {
        const { title, slug, description, comboPrice, primaryProductId, freeProductId, primaryImage, freeImage, hoverImage, badge, tag, includes, stockQuantity, isLimited, sortOrder, } = req.body;
        // Validate products exist
        const [primary, free] = await Promise.all([
            database_1.prisma.product.findUnique({ where: { id: primaryProductId }, select: { finalPrice: true } }),
            database_1.prisma.product.findUnique({ where: { id: freeProductId }, select: { finalPrice: true } }),
        ]);
        if (!primary || !free) {
        ...
        const combo = await database_1.prisma.comboProduct.create({
            data: {
                title,
...
// ADMIN: Update combo
// ============================================================
router.put('/admin/:id', auth_1.protect, (0, auth_1.authorize)('ADMIN'), async (req, res, next) => {
    try {
        const { id } = req.params;
        const updates = req.body;
        // If price or products changed, recalculate savings
        if (updates.comboPrice || updates.primaryProductId || updates.freeProductId) {
            const existing = await database_1.prisma.comboProduct.findUnique({ where: { id } });
            if (!existing) {
                return res.status(404).json({ success: false, message: 'Combo not found' });
            }
            ...
            const [primary, free] = await Promise.all([
                database_1.prisma.product.findUnique({ where: { id: primaryId }, select: { finalPrice: true } }),
                database_1.prisma.product.findUnique({ where: { id: freeId }, select: { finalPrice: true } }),
            ]);
            if (primary && free) {
                const originalTotal = Number(primary.finalPrice) + Number(free.finalPrice);
        ...
        const combo = await database_1.prisma.comboProduct.update({
            where: { id },
            data: updates,
            include: {
                primaryProduct: { select: { id: true, name: true, slug: true, finalPrice: true } },
...
// ADMIN: Delete combo
// ============================================================
router.delete('/admin/:id', auth_1.protect, (0, auth_1.authorize)('ADMIN'), async (req, res, next) => {
    try {
        const { id } = req.params;
        await database_1.prisma.comboProduct.delete({ where: { id } });
