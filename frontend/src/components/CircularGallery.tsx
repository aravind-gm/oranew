    ...
    this.program = new Program(this.gl, {
      ...
      vertex: `
        ...
        void main() {
          vUv = uv;
          vec3 p = position;
          // REDUCED SHAKING: Minimal wave effect only on speed
          p.z = (sin(p.x * 0.8 + uTime * 0.5) * 0.3 + cos(p.y * 0.4 + uTime * 0.3) * 0.3) * (0.02 + uSpeed * 0.1);
...
class App {
  ...
  createMedias(items: any, bend = 1, textColor: string, borderRadius: number, font: string) {
    const defaultItems = [
      { image: `https://picsum.photos/seed/1/800/600?grayscale`, text: 'Necklaces' },
      { image: `https://picsum.photos/seed/2/800/600?grayscale`, text: 'Earrings' },
      { image: `https://picsum.photos/seed/3/800/600?grayscale`, text: 'Bracelets' },
      { image: `https://picsum.photos/seed/4/800/600?grayscale`, text: 'Rings' }
    ];
  ...
  onTouchDown(e: MouseEvent | TouchEvent) {
    ...
    this.start = e instanceof TouchEvent ? e.touches[0].clientX : (e as MouseEvent).clientX;
  }
  ...
  onClick() {
    // Check if user was dragging or just clicking
    if (Math.abs(this.scroll.current - this.scroll.last) < 1 && this.medias) {
      // Find centered item
      const centerItem = this.medias.find((media: any) => {
        const distance = Math.abs(media.plane.position.x - this.scroll.current);
        return distance < media.width / 2;
      });
      
      if (centerItem && this.onItemClick) {
        this.onItemClick({
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    const app = new App(containerRef.current, { 
      ...
      scrollSpeed, 
      scrollEase,
      onItemClick: onItemClick || ((item: any) => {
        ...
        const category = categoryMap[item.text] || item.text.toLowerCase();
  ...
  }, [items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase, onItemClick, router]);
