...
const r2_1 = require("../src/config/r2");
const image_service_1 = require("../src/services/image.service");
...
// VERIFY CDN ACCESS
// ============================================
async function verifyCdnAccess(cdnUrl) {
    try {
        const response = await axios_1.default.head(cdnUrl, { timeout: 10000 });
...
// ============================================
async function migrateProductImage(image, config) {
    ...
    const validation = await (0, image_service_1.isValidImage)(buffer);
    if (!validation.valid) {
        return { success: false, error: validation.error };
    }
    // Step 3: Generate variants and upload to R2
    try {
        const variants = await (0, image_service_1.generateProductVariants)(buffer);
        const uploadedUrls = {};
        for (const variant of variants) {
            const path = (0, r2_1.generateProductImagePath)(productId, variant.role);
            // Check if already exists in R2
            const exists = await (0, r2_1.fileExistsInR2)(path);
            if (exists && config.skipVerified) {
                log('INFO', `Variant ${variant.role} already exists in R2`);
                uploadedUrls[variant.role] = (0, r2_1.getCdnUrl)(path);
                continue;
            }
...
async function runMigration(config) {
    ...
    const supabaseImages = images.filter((img) => img.imageUrl.includes('supabase.co'));
